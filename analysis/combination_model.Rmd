...
for(i in chains){
    filepath <- paste("../model/output/spin-within-model-prediction-group-25000_burn250000_lag9_chain",i,".csv",sep="")
    
    a <- fread(filepath)
    
    a %>%filter(a == "prediction")%>%saveRDS(paste("../model/output/comb_model_predictions_",i,".rds",sep=""))
    
    a %>%filter(a == "globalPrediction")%>%saveRDS(paste("../model/output/comb_model_global_predictions_",i,".rds",sep=""))
    
    a %>%filter(a == "parameter")%>%saveRDS(paste("../model/output/comb_model_parameters_",i,".rds",sep=""))
...
model_predictions <- bind_rows(
  readRDS("../model/output/comb_model_predictions_1.rds"),
  readRDS("../model/output/comb_model_predictions_2.rds"),
  readRDS("../model/output/comb_model_predictions_3.rds"),
  readRDS("../model/output/comb_model_predictions_4.rds"),
  readRDS("../model/output/comb_model_predictions_5.rds")
  )
...
model_parameters <- bind_rows(
  readRDS("../model/output/comb_model_parameters_1.rds"),
  readRDS("../model/output/comb_model_parameters_2.rds"),
  readRDS("../model/output/comb_model_parameters_3.rds"),
  readRDS("../model/output/comb_model_parameters_4.rds"),
  readRDS("../model/output/comb_model_parameters_5.rds")
  )
...
ggplot(chance_global, aes(x = model, y = mean))+
  geom_hline(yintercept = 0.5, lty = 2, alpha = .5)+
  geom_dotplot(data = chance_id_global, binaxis='y', stackdir='center',
               stackratio=1.5, dotsize=.5, alpha = .2)+
  geom_pointrange(aes(ymin = ci_lower, ymax = ci_upper, col = model), size = 0.6)+
  ylim(0,1)+
  facet_grid(~subage)+
...
ggplot(chance_comp, aes(x = model, y = mean))+
  ...
  facet_grid(~subage)+
  labs(x = "", y = "Proportion correct predictions")+
  scale_color_ptol(name = "Model Type")+
  theme_few()
```
...
cor_plot <-  bind_rows(model_binned %>% left_join(data_binned)
  ...
  mutate(subage = ifelse(subage == 3, "3-year-olds", "4-year-olds"))%>%
  ungroup()%>%
  add_row(model = "combination", subage = "2-year-olds")
