...
log_id <- read_excel("../../../../Cloud/spin-within/spin-within_subject_list.xlsx")%>%
  ...
  rename(subid = ID,
         ...
         age = age_num, 
         sex = gender)%>%
  select(subid, subage,age,sex,status)
...
data <-  valid_data%>%
  ...
  filter(status == "keep", 
         !grepl("train",trial))%>%
  select(-target, -test_date, -status)%>%
  mutate(age_num = age, 
         age = age - min(age),
...
# 
## add missing combination data for 40105 (correct = NA)
missing_data_40105 <-  data%>%
  filter(subid == "65507", task == "combination")%>%
  filter(trial >8)%>%
  select(subid, task, familiar, correct, condition)%>%
  mutate(subid = 40105, 
         age = data%>%filter(subid == 40105, task == "combination")%>%pull(age)%>%unique(),
         correct = NA)

## add missing combination data for 55747 (correct = NA)
missing_data_55747 <-  data%>%
  filter(subid == "65507", task == "combination")%>%
  filter(trial >8)%>%
  select(subid, task, familiar, correct, condition)%>%
  mutate(subid = 55747, 
         age = data%>%filter(subid == 55747, task == "combination")%>%pull(age)%>%unique(),
         correct = NA)

# # 
data_model <- bind_rows(data, missing_data_40105,missing_data_55747)
# # 
write(toJSON(data_model), file = "../data//merged_data.json")
```
...
ggplot()+
  ...
  geom_jitter(data = p1, aes(x = subage, y = mean), alpha = .25, width = .05, height = .01)+
  geom_pointrange(data = p2, aes(x = subage, y = mean, ymin = ci_lower, ymax = ci_upper, col = factor(subage)), position = position_dodge(width = .5))+
  facet_grid(~task)+
...
ggplot()+
  ...
  geom_pointrange(data = pc, aes(x = familiar, y = mean, ymin = ci_lower, ymax = ci_upper, col = factor(subage)), position = position_dodge(width = .9))+
