...
We then compared the model's prediction for a trial to the data by simply asking whether the two matched (coded as `1`) or not (coded as `0`). The number of matches (correct predictions) was then evaluated in three ways: 

First, we compared the proportion of correct predictions to chance to see if the model predictions were better than random guessing (50% correct predictions). Figure \@ref(fig:chanceplot) shows that the *integration* model made correct predictions at a level clearly above chance for both age groups (roughly correct in 70% of cases). In a second step, we compared the predictions made by the *integration* model to those of the two lesioned models. For this, we generated trial and child specific model predicitons for each of the lesioned models in the same ways as for the *integration* model. Here we saw that both models made predcitions above chance, confiming that these are valuable alternative models. However, we also saw that the *integration* model was more likely to make correct predictions. 
...
Finally, we looked at the "hits" and "misses", that is, which responses the model was more likely to predict correctly and which not. Figure \@ref(fig:hitplot) showed that the *integration* model was correct in almost all cases when the outcome was `1`. When the outcome was `0`, it was correct in only 50% of cases.  
...
## Participant specific BayesFactors

Quantify evidence in favor of the integrtion model on an individual level - compute a Bayes factor for each model comparison for each child. Look at how
