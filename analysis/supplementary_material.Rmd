...
We used the same tasks as in Bohn, Tessler, Merrick and Frank (2021) to measure children's sensitivity to the different information sources, that is:
...
Across all tasks (except novelty, of course) we used a total of 16 familiar objects. There was one trial for each familiar object in mutual exclusivity, production and comprehension. For the 16 familiar objects, the level of familiarity varied. Four objects (duck, bread, carrot, apple) were retained from Bohn et al. (2021) and can be considered highly familiar objects for 3- and 4-year-old children. Twelve additional objects were modeled after a previous mutual exclusivity study involving German 3- and 4-year-olds (Grassmann et al., 2015). Some of these objects (e.g., corkscrew) are barely familiar for children in this age group. 

#### Training 

Generally, each child participated in two testing sessions. Both sessions began with a brief training. First, the child was instructed to point to colorful dots on the screen, which disappeared when their parent selected them. This way the child could practice pointing at the screen and the parent could practice interpreting their child's pointing gestures. Next, there were two training trials where animal speakers requested highly familiar objects (duck and teddy bear) from the child. This introduced children to the procedure of the following tasks, where animals requested objects from them and they had to point to the requested object. 

#### Mutual exclusivity experiment 

Each child participated in the mutual exclusivity experiment (16 trials) on the first testing day. Here, animal speakers stood on a small hill between two tables on which objects were placed. One of these objects was potentially familiar to the child, the other object was unknown (retained from Bohn et al., 2020 or designed for the purpose of this study). After introducing themselves, speakers requested objects from the child using novel labels label (e.g., “Oh, cool, da liegt ein Höfas auf dem Tisch, wie toll! Ein Höfas liegt auf dem Tisch! Kannst du mir das Höfas geben?” [“Oh, cool, there is a höfas on the table, how neat! A höfas is on the table. Can you give me the höfas?”]). 
...
To evaluate the model predictions, we first replicated the group--level results of @bohn2021young. The goal was to show that a) model predictions are highly correlated with the data and b) the *integration* model makes better predictions compared to a set of alternative models. Next, we turned to the focal participant--level analysis and evaluated how well the *integration* model predicted the trial--by--trail performance of each child, both on an absolute level and in comparison to the alternative models. 

### Alternative models

In addition to the *integration* model, we considered two alternative, lesioned models. Both models represent thy hypothesis that children focus on one type of inference (mutual exclusivity or common ground) whenever multiple inferences are possible. The predictions of these alternative models were generated using the same parameters as the *integration* model, the difference lies in which parameters were used. 
...
First, we averaged across predicitons for each participant and compared the proportion of correct predictions across participants to a level expected by random guessing (50% correct predictions). Figure \@ref(fig:chanceplot) shows that the *integration* model made correct predictions at a level clearly above chance for both age groups (roughly correct in 70% of cases). In a second step, we compared the predictions made by the *integration* model to those of the two lesioned models. For this, we generated trial and child specific model predictions for each of the lesioned models in the same ways as for the *integration* model. Here we saw that both models made predictions above chance, confirming that these were valuable alternative models. However, we also saw that the *integration* model was more likely to make correct predictions. 
...
chance_id%>%
  ...
  mutate(row = 1:n())%>%
  pivot_wider(names_from = row, values_from = model)%>%
  mutate(win_model = paste(`1`,`2`, sep = " & "),
         win_model = str_remove(win_model, " & NA"))%>%
  ungroup()%>%
  group_by(win_model)%>%
  summarise(n = n())%>%
  ggplot(aes(x = win_model, y = n))+
  geom_bar(stat = "identity")+
  theme_few()+
  labs(y = "No. of Individuals (out of 60)", x = "Model(s) with best predictions")+
  theme(axis.text.x=element_text(angle = 45, vjust = 1, hjust = 1))

```
```{r}
ggsave("../graphs/win_model_id.png", width = 6, height = 4, scale = 1)
```

Finally, we looked at the "hits" and "misses", that is, which responses the model was more likely to predict correctly and which not. Figure \@ref(fig:hitplot) showed that the *integration* model was correct in almost all cases when the outcome was `1`. When the outcome was `0`, it was correct in about 50% of cases.  
...
For this final analysis, we repeated the model comparison on an individual level. That is, for each participant, we computed a Bayes factor (BF) in favor of the *integration* model compared to the two lesioned models. Figure \@ref(fig:parytayes) visualizes the distribution of the resulting BFs on the log-scale. Log-BFs larger than 0 correspond to BFs > 1 and thus suggest that the *integration* model fit the data better compared to the other model for this participant. Taken together, `r xx` % of log-BFs were larger than 0 when comparing the *integration* model to the *no common ground* model and `r ` % of log-BFs were larger than 0 when comparing the *integration* model to the *no speaker informativeness* model. this suggests that for the majority of participants, the *integration* model was the best model to predict their behavior in a new situation. 
