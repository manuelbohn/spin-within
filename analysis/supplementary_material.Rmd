...
First, we averaged across predicitons for each participant and compared the proportion of correct predictions across participants to a level expected by random guessing (50% correct predictions). Figure \@ref(fig:chanceplot) shows that the *rational integration* model made correct predictions at a level clearly above chance for both age groups (roughly correct in 70% of cases). In a second step, we compared the predictions made by the *rational integration* model to those of the two lesioned models. For this, we generated trial and child specific model predictions for each of the lesioned models in the same ways as for the *rational integration* model. Here we saw that both models made predictions above chance, confirming that these were valuable alternative models. However, we also saw that the *rational integration* model was more likely to make correct predictions. 
...
Finally, we looked at the "hits" and "misses", that is, which responses the model was more likely to predict correctly and which not. Figure \@ref(fig:hitplot) showed that the *rational integration* model was correct in almost all cases when the outcome was `1`. When the outcome was `0`, it was correct in about 50% of cases.  
