...
```{r corplot, fig.cap = "Correlation between model predictions and data (aggregated across individuals and binned by year with 95\\%HDI) for each trial in the combination experiment for the rational integration model (left) and the two alternative models (right).",fig.height = 4, fig.width= 10, out.width="100%"}
...
```{r loglikeplot, fig.cap = "Log-likelihood of the data given each of the models in the model comparison.",fig.height = 2.5, fig.width= 10, out.width="100%"}
lp
```
...
Below we visualize the posterior distributions for some of the parameters in the model (speaker informativeness: Figure \@ref(fig:speakoptplot); semantic knowledge: Figure \@ref(fig:semknowplot); sensitivity to common ground: Figure \@ref(fig:cgplot)). Finally, we show the posterior distribution of the production probability parameter (Figure \@ref(fig:prodprob)). This parameter was part of the production model and gives the probability that the child was able to produce a word if their semantic knowledge indicates that they knew it. This parameter was the same for all participants. 
