...
A defining feature of human communication is its flexibility. Conventional languages -- signed and spoken -- allow for expressing a near-infinite number of messages. In the absence of a shared language, humans can produce and understand novel signals which can rapidly be transformed into structured communication systems [@bohn2019young; @brentari2017language; @fay2018create]. The flexibility stems from a powerful social-cognitive infrastructure that underlies human communication [@tomasello2008origins; @sperber2001relevance; @levinson2014origin]. Interlocutors can recruit and integrate a range of different information sources -- conventional language being one of them -- to make so-called *pragmatic* inferences about the speaker’s intended meaning in context [@grice1991studies]. They play an important role during everyday language use [@clark1996using; @schulze2021children] and during language acquisition [@bohn2019pervasive; @clark2009first; @tomasello2009constructing]. 

Decades of developmental research have shown that children readily make pragmatic inferences in a wide variety of contexts and starting at an early age [@bohn2019pervasive; @schulze201518]. For example, already early in the second year of life, children use their emerging semantic knowledge (word-object mappings) to infer that a speaker uses a novel word to refer to a novel object [@pomiechowska2021twelve; @markman1988children; @halberda2003development; @bion2013fast; @merriman1989mutual; @clark1988logic; @lewis2020role]. Around the same age, children start to use common ground (shared knowledge) in communication [@bohn2018common; @akhtar1996role; @diesendruck2004two; @bohn2018social; @ganea2007infants]. From age three onwards, they expect speakers to communicate in an informative and context-sensitive way [@frank2014inferring; @schulze20133; @schulze2022context]. 
...
The field of pragmatic language comprehension has been particularly active from a computational modeling perspective [@cummins2014computational], including work on common ground [@anderson2021tell; @heller2016perspective], politeness [@yoon2020polite]; over-informativeness [@degen2020redundancy]; implicature [@franke2020theory], and generic language [@tessler2019language]. The Rational Speech Act (RSA) framework has been one productive framework for modeling pragmatic inference, construing language understanding as a special case of Bayesian social reasoning [@frank2012predicting; @goodman2016pragmatic; @scontras2021practical]. RSA models are distinguished by their recursive structure in which a listener reasons about a cooperative speaker -- sensu @grice1991studies -- who reasons about a literal listener who interprets words according to their literal semantics. These models have been successfully applied to predict aggregate behavior -- the average judgment probability across a large group of participants, for example -- for a range of different pragmatic phenomena [reviewed in @frank2012predicting; @goodman2016pragmatic].
...
```{r}
# speak_opt <- readRDS("../analysis/saves/speak_opt_plot.rds")
# cg <- readRDS("../analysis/saves/com_ground_plot.rds")
# sem_know <- readRDS("../analysis/saves/sem_know_plot.rds")
# 
# bind_rows(
# speak_opt%>%
...
#             lci = hdi_lower(value),
#             uci = hdi_upper(value))%>%
#   mutate(type = "speaker_optimality"),
# 
# 
# cg%>%
#   filter(parameter != "sigma")%>%
...
#             lci = hdi_lower(value),
#             uci = hdi_upper(value))%>%
#   mutate(type = "semantic_knowledge")
# 
# 
# )%>%write_csv("../analysis/saves/parameter_aggregates.csv")

params_agg <- read_csv("../analysis/saves/parameter_aggregates.csv")

aoa <- read_csv("../analysis/saves/age_of_acquisition.csv")
...
The hierarchical nature of the parameter estimation procedure in our model allowed us to take an aggregate look at these results in what they indicate about the development of sensitivity to the different information sources. For this, we extracted the posterior distributions for intercepts and slopes for the parameter estimates corresponding to the different information sources ($\alpha$, $\rho$, and $\theta$) based on which the participant-specific estimates were sampled. These values can be taken to describe the average developmental trajectory for the respective parameter and with that, the sensitivity to the respective information source. For expectations about speaker informativeness, the intercept was larger than 1 (mode = `r round(params_agg%>%filter(type == "speaker_optimality", parameter == "intercept")%>%pull(mode),2)`; 95% HDI = `r round(params_agg%>%filter(type == "speaker_optimality", parameter == "intercept")%>%pull(lci),2)` - `r round(params_agg%>%filter(type == "speaker_optimality", parameter == "intercept")%>%pull(uci),2)`) and the slope was positive (mode = `r round(params_agg%>%filter(type == "speaker_optimality", parameter == "slope")%>%pull(mode),2)`; 95% HDI = `r round(params_agg%>%filter(type == "speaker_optimality", parameter == "slope")%>%pull(lci),2)` - `r round(params_agg%>%filter(type == "speaker_optimality", parameter == "slope")%>%pull(uci),2)`) suggesting that already the youngest children (age was standardized so that minimum age was 0) were expecting the speaker to be informative and this expectation increased with age. For sensitivity to common ground, the intercept was positive (mode = `r round(params_agg%>%filter(type == "prior", parameter == "intercept")%>%pull(mode),2)`; 95% HDI = `r round(params_agg%>%filter(type == "prior", parameter == "intercept")%>%pull(lci),2)` - `r round(params_agg%>%filter(type == "prior", parameter == "intercept")%>%pull(uci),2)`) while the slope was negative (mode = `r round(params_agg%>%filter(type == "prior", parameter == "slope")%>%pull(mode),2)`; 95% HDI = `r round(params_agg%>%filter(type == "prior", parameter == "slope")%>%pull(lci),2)` - `r round(params_agg%>%filter(type == "prior", parameter == "slope")%>%pull(uci),2)`) showing that sensitivity to common ground was very high at 3 years of age (probability to select the discourse-novel object = $logistic$(1.96) = 0.88) and slightly decreased with age. For semantic knowledge, the intercept and slope represent the overall vocabulary development independent of particular familiar word--object pairings (conditional on the familiar objects involved in the study). At 3 years of age, the average probability to know the label for a word was 0.23 ($logistic$(-1.21); intercept estimate: mode = `r round(params_agg%>%filter(type == "semantic_knowledge", parameter == "intercept")%>%pull(mode),2)`; 95% HDI = `r round(params_agg%>%filter(type == "semantic_knowledge", parameter == "intercept")%>%pull(lci),2)` - `r round(params_agg%>%filter(type == "semantic_knowledge", parameter == "intercept")%>%pull(uci),2)`), which substantially increased with age (slope estimate: mode = `r round(params_agg%>%filter(type == "semantic_knowledge", parameter == "slope")%>%pull(mode),2)`; 95% HDI = `r round(params_agg%>%filter(type == "semantic_knowledge", parameter == "slope")%>%pull(lci),2)` - `r round(params_agg%>%filter(type == "semantic_knowledge", parameter == "slope")%>%pull(uci),2)`). To contextualize the semantic knowledge of the different familiar objects, we correlated the probability to know a word (averaged across participants) with age-of-acquisition ratings for English translations these words obtained by @kuperman2012age^[German ratings were not available for all words.]. We found a strong negative correlation of *r* = `r round(cor(aoa$mode, aoa$age_of_acquisition),2)`, suggesting that participants (German children) had less semantic knowledge of words that were rated (by adult English-speakers) to be acquired later in development. 
